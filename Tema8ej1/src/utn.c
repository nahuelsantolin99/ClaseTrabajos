...
int getsValidado(const char* mensaje, const char* mensajeError, char* cadena, int longitud)
{
	int retorno = -1;
	char auxCad[4096];

	if(mensaje != NULL && mensajeError != NULL && cadena != NULL && longitud)
	{
		printf("%s", mensaje);
		fflush(stdin);
		gets(auxCad);
...
int esLetraConEspacio(char *pResultado)
{
	...
	if((strlen(pResultado)/2 == aux || (strlen(pResultado) / 2) - 1 == aux || (strlen(pResultado) / 2) + 1 == aux) && contadorLetras > 1)
	{
		retorno = 1;
